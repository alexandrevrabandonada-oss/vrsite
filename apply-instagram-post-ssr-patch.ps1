...
$backupDir = 'backup-instagram-post-ssr-' + $stamp
New-Item -ItemType Directory -Force -Path (Join-Path $backupDir 'src\app\instagram\[id]') | Out-Null

$src = Join-Path (Get-Location) 'payload\src\app\instagram\[id]\page.tsx'
$dst = Join-Path (Get-Location) 'src\app\instagram\[id]\page.tsx'

if (Test-Path $dst) {
  Copy-Item $dst (Join-Path $backupDir 'src\app\instagram\[id]\page.tsx') -Force
}

New-Item -ItemType Directory -Force -Path (Split-Path $dst) | Out-Null
Copy-Item -Path $src -Destination $dst -Force
Say '==> Atualizado src/app/instagram/[id]/page.tsx'

git add -A
git commit -m 'fix(instagram): detalhe SSR com origem via headers e dynamic=force-dynamic' | Out-Null
