...
touch .gitignore
if ! grep -q '^backup-\*/' .gitignore; then echo 'backup-*/' >> .gitignore; fi
if ! grep -q '^.backups/' .gitignore; then echo '.backups/' >> .gitignore; fi
...
for (const e of ["**/backup-*/**",".backups/**"]) if (!ts.exclude.includes(e)) ts.exclude.push(e);
fs.writeFileSync('tsconfig.json', JSON.stringify(ts, null, 2));
NODE
fi

# 4) Remover pastas backup-*
for d in backup-*; do
  [ -d "$d" ] || continue
  say "==> Removendo $d"
  git rm -r --cached --ignore-unmatch -q "$d" || true
  rm -rf "$d"
done
...
git add -A
git commit -m "chore(cleanup): excluir backups do repo, ajustar ignore e tsconfig" >/dev/null || true
