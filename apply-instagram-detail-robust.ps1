...
$backup = "backup-instagram-detail-robust-$stamp"
if (Test-Path $dst) {
  New-Item -ItemType Directory -Force -Path (Join-Path $backup 'src\app\instagram\[id]') | Out-Null
  Copy-Item $dst (Join-Path $backup 'src\app\instagram\[id]\page.tsx') -Force
}
...
git add -A
git commit -m "fix(instagram): detalhe com fetch relativo + fallback absoluto e debug rico" | Out-Null
