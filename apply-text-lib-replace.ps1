...
$src = Join-Path (Get-Location) 'payload\src\lib\text.ts'
$dst = Join-Path (Get-Location) 'src\lib\text.ts'
...
git add -A
git commit -m 'fix(text): substituir src/lib/text.ts por versão compatível (sem flag u)' | Out-Null
