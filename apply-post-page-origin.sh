...
backupDir="backup-postpage-origin-$stamp"
...
git add -A
git commit -m "fix(posts): resolver origem no servidor via headers; dynamic=force-dynamic" >/dev/null || true
...
say "==> Post Page Origin Hotfix aplicado."
