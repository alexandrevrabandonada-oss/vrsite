...
# Ensure import
if ! grep -q "from '@/components/HomeSearchBar'" "$homeFile" && ! grep -q 'from "@/components/HomeSearchBar"' "$homeFile"; then
  sed -i.bak "1s;^;import HomeSearchBar from '@/components/HomeSearchBar'\n;" "$homeFile" && rm -f "$homeFile.bak"
fi

# Insert before first <main>, or after "return ("
if grep -q "<main" "$homeFile"; then
  awk 'BEGIN{done=0} { if (!done && match($0, /<main/)) { print "<HomeSearchBar />"; done=1 } print }' "$homeFile" > "$homeFile.tmp" && mv "$homeFile.tmp" "$homeFile"
elif grep -q "return (" "$homeFile"; then
  awk 'BEGIN{done=0} { if (!done && match($0, /return *\(/)) { print; print "  <HomeSearchBar />"; done=1; next } }1' "$homeFile" > "$homeFile.tmp" && mv "$homeFile.tmp" "$homeFile"
else
  printf "\n<HomeSearchBar />\n" >> "$homeFile"
fi
...
git add -A
git commit -m "fix(ui): injetar HomeSearchBar sem regex problematica e evitar \$HOME" >/dev/null || true
