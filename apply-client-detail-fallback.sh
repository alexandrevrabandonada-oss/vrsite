...
backupDir="backup-client-detail-$stamp"
mkdir -p "$backupDir"
...
# Corrige alias
for f in "src/app/instagram/[id]/page.tsx" "src/components/HomeFeed.tsx"; do
  [ -f "$f" ] || continue
...
git add -A
git commit -m "feat(instagram): detalhe client-side em /instagram/[id] + links na Home" >/dev/null || true
...
say "==> Client Detail Fallback aplicado."
