...
async function getItem(id: string) {
  ...
  const IG_BASE =
    process.env.INSTAGRAM_GRAPH_BASE || "https://graph.facebook.com/v20.0";
  ...
  const res = await fetch(
    `${IG_BASE}/${id}?fields=${fields}&access_token=${encodeURIComponent(token)}`,
    { cache: "no-store" },
  );
  ...
  return (
    <main className="px-4 py-8 mx-auto max-w-3xl">
      ...
      <article className="mt-4 space-y-4">
        ...
        {item.caption && (
          <p className="whitespace-pre-wrap text-zinc-200">{item.caption}</p>
        )}
        <a
          href={item.permalink}
          target="_blank"
